...
public class DAO 
{
	...
	public int insert(RegisterBean rb) throws ClassNotFoundException, SQLException
	{
		...
		Statement st2=con.createStatement();
		st2.execute("use professor");
		PreparedStatement p=con.prepareStatement("select * from register");
		ResultSet rs=p.executeQuery();
		while(rs.next())
		{
			if(rs.getInt(1)==rb.getId()|| rs.getString(3).equals(rb.getEmail()))
			{
				return 0;
			}
		}
	...
	public int sendfeedback(String email,String feedback) throws ClassNotFoundException, SQLException
	{
		...
		PreparedStatement ps2=con.prepareStatement("select * from feedback");
		ResultSet rs2=ps2.executeQuery();
		while(rs2.next())
		{
			if(rs2.getInt(1)==id)
			{
				PreparedStatement ps3=con.prepareStatement("update feedback set feedback=? where id=?");
				ps3.setString(1, feedback);
				ps3.setInt(2, id);
